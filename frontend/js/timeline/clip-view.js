...
export class ClipView {
  static drawClip(ctx, clip, trackColor, peakData, trackY, trackHeight, zoom, isSelected = false, hoverEdge = null) {
    ...
    if (peakData && peakData.levels) {
      ...
      if (level && level.channels && level.channels.length > 0) {
        ...
        ctx.clip(); // Clip strictly inside waveform region
    ...
    // 6. Edge Trim Handles (visual cues)
    const handleW = 6;
